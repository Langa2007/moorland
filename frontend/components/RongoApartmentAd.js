    ...
    <section className="section-pad bg-charcoal text-ivory">
      <div className="luxury-container grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <div>
          ...
          <h2 className="heading-lg mt-3">{advert.title || "Rongo Apartment"}</h2>
          <p className="mt-5 text-lg leading-8 text-cream/80">
            {advert.description || "A dedicated advertisement space for the upcoming Rongo Apartment property."}
          </p>
        ...
        <div className="grid gap-3">
          <img src={primaryImage} alt={`${advert.title || "Rongo Apartment"} featured view`} className="aspect-[16/10] w-full rounded-lg object-cover shadow-soft" loading="lazy" />
          {images.length > 1 ? (
            <div className="grid grid-cols-3 gap-3">
              {images.slice(1, 4).map((image, index) => (
                <img key={`${image}-${index}`} src={image} alt={`${advert.title || "Rongo Apartment"} preview ${index + 2}`} className="aspect-[4/3] w-full rounded-lg object-cover" loading="lazy" />
              ))}
            </div>
          ) : (
